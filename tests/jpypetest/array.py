...
class ArrayTestCase(common.JPypeTestCase):
    ...
    def testIterateArray(self):
        ...
        # <AK> added for test (deprecated) JIterator
        for i in JIterator(t.i):
            self.assertNotEqual(i, 0)
    ...
    def testJArrayConversionShort(self):
        ...
        self.VALUES = [v for v in self.VALUES if v < (2**16 // 2 - 1)  # <AK> was: 2**16 / 2
                       and v > (2**16 // 2 * -1)]                      # <AK> was: 2**16 / 2
        ...
        #    jpype.JArray(jpype.JShort)([2**16//2])  # <AK> was: 2**16/2
